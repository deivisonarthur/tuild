...
JS = {
	...
	commands:{
		...
		'-h': function(){
			...
			console.log(
				...
				'-min                   [DEFAULT OPTION]. return the minified code of your js file(s). If a destination is specified, is created/edited the file destination \n' + 
	...
	task: function(type, src, dest, callback){
		...
		if(JS.__watch){
			...
			return TUILD.watch(src, 'js', function(file, date){
				
				TUILD.log('JS: Changed file: ' + file + ' on ' + TUILD.formatDate(date));
	...
	/**
	 * Create the minified version of JS code
	 * @param [string - array] src
	 * @param [string] dest
	 */
