...
Tuild = {
	...
	/**
	 * Format one date and return the result
	 * @param [date] date
	 */
	formatDate: function(date){
		
		date = (date.constructor == Date)? date : new Date();
		return (date.getFullYear() + '/' + date.getMonth() + '/' + date.getDate() + ' at ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds());
	},
	...
	task: function(src, dest, callback, messageInit, messageSucess, messageFail){
		...
		if(dest){
			...
			Tuild.log(messageSucess + ' [' + PATH.resolve(process.cwd(), dest) + ']');
		...
		files.forEach(function(file){
			...
			FS.watchFile(file, function(current, previous){
				...
				if(current.mtime.getTime() > previous.mtime.getTime()){
					callback.call(null, file, current.mtime);
				}
		 ...
		 */
			...
			if(src.constructor === Array){
				...
				src.forEach(function(file){
					itens.push( PATH.resolve(process.cwd(), file) );
				});
			 ...
			 */
				...
				itens.push( PATH.resolve(process.cwd(), src) );
			}
	...
	/**
	 * Create the minified version of code
	 * @param [string - array] src
	 * @param [string] dest
	 * @param [string] namespace
	 * @param [function] callback
	...
	min: function(src, dest, namespace, callback){
		...
		return Tuild.task(
			...
			dest, 
			callback, 
			(namespace + ': Creating minified code.'),
			(namespace + ': Minified created successfully!'),
			(namespace + ': Error to create minified.')
		);
